...
class StockHeteroGNN(nn.Module):
    ...
    def __init__(self, stock_features: int = 34, comment_features: int = 768, 
        ...
        # 批归一化
        self.batch_norm_stock = nn.BatchNorm1d(hidden_dim)
        self.batch_norm_comment = nn.BatchNorm1d(hidden_dim)
...
def create_sample_data():
    ...
    # 创建单日的异构图数据
    def create_daily_data(num_stocks=3):
        data = HeteroData()
        
        # 股票节点特征 (3只股票，34维特征)
        data['stock'].x = torch.randn(num_stocks, 34)
        
        # 评论节点特征 (假设每只股票有5条评论)
        num_comments = num_stocks * 5
        data['comment'].x = torch.randn(num_comments, 768)
        
        # 边索引 - 股票与评论的连接
        stock_comment_edge_index = torch.randint(0, num_stocks, (2, num_comments))
        data['stock', 'connect', 'comment'].edge_index = stock_comment_edge_index
        
        # 反向边
        comment_stock_edge_index = torch.stack([stock_comment_edge_index[1], stock_comment_edge_index[0]], dim=0)
        data['comment', 'connect_rev', 'stock'].edge_index = comment_stock_edge_index
        
        return data
    
    # 创建15天的数据序列
    sequence_data = [create_daily_data() for _ in range(15)]
    
    # 创建目标值
    targets = {
        'volatility_5d_target': torch.rand(1, 1),  # 5日波动率
        'direction_5d_target': torch.randn(1, 1),  # 5日方向
        'volatility_30d_target': torch.rand(1, 1),  # 30日波动率
        'direction_30d_target': torch.randn(1, 1)   # 30日方向
    }
    
    return sequence_data, targets
...
def train_model():
    ...
    """
    # 创建模型
    model = StockHeteroGNN(stock_features=34, comment_features=768, hidden_dim=256, num_layers=3)
    
    # 创建训练数据
    sample_data, targets = create_sample_data()
    
    # 创建数据加载器（这里简化为单个批次）
    train_dataloader = [(sample_data, targets)]  # 实际使用中应该是DataLoader
    ...
    for epoch in range(10):
        ...
        # 评估
        eval_loss = trainer.evaluate(train_dataloader)
